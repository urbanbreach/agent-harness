#!/usr/bin/env bash
# Capture SHELL-QUESTION/OVL-QUESTION L3 (question-tool-in-flight streaming state).
# Matches reference freeze run1-shell-question-pinned-v1 (120x40).
#
# Output:
#   artifacts/qa-evidence/20260717-tui-reference-parity/actual/harness-shell-question-pinned-v1/
...
EVIDENCE_DIR="${EVIDENCE_DIR:-artifacts/qa-evidence/20260717-tui-reference-parity/actual/harness-shell-question-pinned-v1}"
COLS=120
ROWS=40
FONT_SIZE=15
...
export HARNESS_TUI_PTY_HELPER_SCENARIO=live_question_stream
...
CMD="$BIN --exact pty_helper_live_question_stream --nocapture"

echo "Capturing question-stream L3 -> $EVIDENCE_DIR (120x40, fontSize $FONT_SIZE)"
node scripts/tui-parity/web-terminal-visual-qa.mjs \
  --title "harness-shell-question-pinned-v1" \
  --command "$CMD" \
  --source-label "pty_helper:live_question_stream" \
  --cols "$COLS" \
  --rows "$ROWS" \
  --font-size "$FONT_SIZE" \
  --dwell-ms "$DWELL_MS" \
...
if ! grep -q 'Waiting for response' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: L3 missing 'Waiting for response' status" >&2
  exit 1
fi
if ! grep -q 'ask me the parity question' "$EVIDENCE_DIR/terminal.txt"; then
...
echo "OK: question-stream L3 at $EVIDENCE_DIR"
ls -la "$EVIDENCE_DIR"
