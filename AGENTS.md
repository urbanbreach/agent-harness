...
├── crates/harness-tools/     # native tools: fs/edit/bash/task/web/code/lsp/mcp/session/team/control plane
...
| Native tools | `crates/harness-tools/AGENTS.md` | Read before changing tool ids, schemas, path safety, bash, MCP, LSP, task/session/team tools. |
...
|------|------|------------------|
| `harness-core::coord` | Single scheduling/event/permission/hook/lifecycle authority | `cargo test -p harness-core --test coord_test` |
| `harness-core::event` | Event schema v1 and append-only envelopes | `cargo test -p harness --test event_docs_reference_test` |
| `harness-core::config` | Runtime/TUI config loading, validation, public schema shape | `cargo test -p harness --test config_schema_cli_test` |
| `harness-providers` | Streaming provider boundary, redacted metadata, cassette replay | `cargo test -p harness-providers` |
| `harness-tools` | Built-in native tool surface and schema parity | `cargo test -p harness-tools --test native_tool_parity_matrix_test` |
| `harness-tui` | App state, view models, renderer, shell geometry | `cargo test -p harness-tui --test deterministic_render_test` |
| `harness-testkit` | Fakes, simulation evidence, visual/PTY helpers | `cargo test -p harness-testkit --test simulation_validator_test` |
