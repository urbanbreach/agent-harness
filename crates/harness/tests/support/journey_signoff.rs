...
fn assert_config_journey_evidence(row: &Value, journey_id: &str, l3: &str, l6: &str, strict: bool) {
    ...
    // lane regenerates fresh L3+L6 evidence under a new evidence root
    ...
    assert_eq!(
        row["status"].as_str(),
        Some("incomplete"),
        "{journey_id} must stay incomplete until the strict signoff lane regenerates fresh L3+L6 evidence"
    );
...
pub(crate) fn assert_all_journey_manifest_evidence_with(rows: &[Value], strict: bool) {
    ...
    // Wave 1 honesty: worktree journey stays incomplete until fresh L3+L6
    // evidence is regenerated by the strict signoff lane (Contract §4).
    assert_eq!(
        worktree["status"].as_str(),
        Some("incomplete"),
        "worktree journey must stay incomplete until the strict signoff lane regenerates fresh L3+L6 evidence"
    );
