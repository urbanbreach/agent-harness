...
| `crates/harness-core/src/coord.rs` remains a giant coordinator module, roughly 10.3k lines. | Large-file scan and coordinator guidance in `crates/harness-core/AGENTS.md`. | **Substantially mitigated; accepted as a staged exception.** Follow-up work extracted child-session materialization, hooks, permissions, provider-context handling, question permissions, task categories, team coordination, tool execution, and tool metadata into `coord/` modules while preserving the coordinator as the single scheduling/event/permission authority. The remaining 9,114-line file is still large, but further decomposition must be driven by concrete duplicated local patterns so the single-authority invariant does not leak. |
...
| `crates/harness-core/src/coord.rs` | Partially addressed at roughly 10.3k lines. | 9,114-line coordinator plus focused `coord/` modules for child sessions, hooks, permissions, provider context, questions, task categories, team flow, tool execution, and tool metadata. | Closed as a staged exception: the remaining size is justified by the coordinator single-authority invariant. |
