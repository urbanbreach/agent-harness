#!/usr/bin/env bash
# Capture draft-composer L3 evidence (P0-START-03 / P0-COMP-01 / P0-KEY-01).
# Reference freeze: run1-draft (120x32, pinned reference binary 883e3dea after
# typing "Browser QA draft" on the welcome panel).
# Harness scenario: type_first_startup helper with scripted keystrokes; typing
# dismisses the welcome panel and the composer retains the draft with the
# freeze footer grammar (Enter:send | Shift+Tab:mode | Ctrl+x:shortcuts).
#
# Output:
#   artifacts/qa-evidence/20260717-tui-reference-parity/actual/harness-draft-v23/
...
EVIDENCE_DIR="${EVIDENCE_DIR:-artifacts/qa-evidence/20260717-tui-reference-parity/actual/harness-draft-v23}"
COLS=120
ROWS=32
FONT_SIZE=15
...
echo "Capturing draft composer L3 -> $EVIDENCE_DIR (120x32, fontSize $FONT_SIZE)"
node scripts/tui-parity/web-terminal-visual-qa.mjs \
  --title "harness-draft-v23" \
  --command "$CMD" \
  --source-label "pty_helper:type_first_startup+draft_input" \
  --input "Browser QA draft" \
  --cols "$COLS" \
  --rows "$ROWS" \
  --font-size "$FONT_SIZE" \
...
# Structural honesty checks (fail closed on draft chrome + footer grammar).
if [[ ! -f "$EVIDENCE_DIR/terminal.png" ]]; then
  echo "FAIL: terminal.png missing" >&2
  exit 1
fi
if ! grep -q 'Browser QA draft' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 missing typed draft text" >&2
  exit 1
fi
if ! grep -q '❯' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 missing composer glyph" >&2
  exit 1
fi
if ! grep -q 'Enter:send' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 missing Enter:send footer grammar" >&2
  exit 1
fi
if ! grep -q 'Shift+Tab:mode' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 missing Shift+Tab:mode footer grammar" >&2
  exit 1
fi
if ! grep -q 'Ctrl+x:shortcuts' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 missing Ctrl+x:shortcuts footer grammar" >&2
  exit 1
fi
if grep -q 'New worktree' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: draft L3 must dismiss welcome action rows" >&2
  exit 1
fi
...
meta["behavior_id"] = "P0-START-03"
meta["viewport"] = {"cols": 120, "rows": 32}
meta["generating_command"] = meta.get("generating_command") or (
    "lane-capture: scripts/tui-parity/capture-draft-composer-l3.sh "
    "(web-terminal-visual-qa.mjs, pty_helper:type_first_startup + draft input)"
)
...
echo "OK: draft composer L3 at $EVIDENCE_DIR"
ls -la "$EVIDENCE_DIR"
