...
5. **Stale state**: old `Harness workflow projection` files, mismatched sessions, missing timestamps, and contradictory phase metadata.
...
| **Environment Error** | Exit: `ULTRAQA ERROR: [native terminal UI/port/dependency/hung command issue]` plus cleanup status |
...
- **On each cycle**:
  `harness workflow evidence write --input '{"mode":"ultraqa","current_phase":"qa","iteration":<cycle>,"scenario_matrix":"<updated matrix path or summary>"}' --json`
- **On adversarial e2e transition**:
  `harness workflow evidence write --input '{"mode":"ultraqa","current_phase":"adversarial-e2e"}' --json`
- **On diagnose/fix transitions**:
  `harness workflow evidence write --input '{"mode":"ultraqa","current_phase":"diagnose"}' --json`
  `harness workflow evidence write --input '{"mode":"ultraqa","current_phase":"fix"}' --json`
- **On cleanup transition**:
  `harness workflow evidence write --input '{"mode":"ultraqa","current_phase":"cleanup"}' --json`
- **On completion**:
  `harness workflow evidence write --input '{"mode":"ultraqa","active":false,"current_phase":"complete","completed_at":"<now>"}' --json`
- **For resume detection**:
  `harness workflow evidence read --input '{"mode":"ultraqa"}' --json`
...
`harness workflow evidence clear --input '{"mode":"ultraqa"}' --json`
