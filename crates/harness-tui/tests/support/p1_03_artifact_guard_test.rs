//! Rejection tests for the P1-03 artifact-root guard. These mirror the
//! security review finding: an environment-supplied root used to flow
//! straight into `remove_dir_all`.
...
fn temporary_home(name: &str) -> PathBuf {
    let directory =
        std::env::temp_dir().join(format!("harness-p1-03-guard-{}-{name}", std::process::id()));
    fs::create_dir_all(&directory).unwrap_or_abort();
    directory
}

#[test]
fn p1_03_artifact_root_rejects_unowned_directories() {
    // arrange
    ...
    fs::write(foreign.join(".harness-p1-03-artifact-root"), b"").unwrap_or_abort();
...
#[test]
fn p1_03_artifact_root_rejects_unsanctioned_canonical_roots() {
...
#[test]
fn p1_03_artifact_root_accepts_sanctioned_roots() {
    // arrange: roots inside repository target/ and the platform tempdir.
    let inside_target = repository_root().join("target/p1-03-guard-accept");
    let inside_temporary =
        std::env::temp_dir().join(format!("harness-p1-03-guard-accept-{}", std::process::id()));
