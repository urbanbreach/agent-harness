...
pub(crate) fn exact_test_operator_rail_section_model_hides_empty_sources_but_preserves_order() {
    ...
    assert_eq!(
        ...
        ["No batch activity yet".to_string()]
    );
...
pub(crate) fn exact_test_operator_rail_section_model_separates_tool_families() {
    let model = build_operator_rail_model(&operator_rail_tool_activity_app());
...
fn operator_sidebar_mcp_lines(app: &AppState) -> Vec<String> {
    let items = operator_sidebar_collect_tool_lines(app, |tool_call| {
        matches!(tool_call.canonical_tool_id(), "search.web" | "search.code")
...
fn operator_sidebar_web_lines(app: &AppState) -> Vec<String> {
    let items = operator_sidebar_collect_tool_lines(app, |tool_call| {
        (tool_call.canonical_tool_id() == "web.fetch")
            .then(|| operator_sidebar_tool_activity_item(tool_call))
    });
    if items.is_empty() {
        vec!["No web activity yet".to_string()]
    } else {
        items
    }
}
...
fn operator_sidebar_batch_lines(app: &AppState) -> Vec<String> {
    ...
    if items.is_empty() {
        vec!["No batch activity yet".to_string()]
    } else {
        items
    }
}
...
fn build_operator_rail_model(app: &AppState) -> OperatorRailModel {
    ...
    let mut sections = vec![
        ...
        OperatorRailBodySection::Mcp {
            count: operator_sidebar_section_count(&mcp_lines, "No MCP activity yet"),
            items: mcp_lines,
        },
        OperatorRailBodySection::Web {
            count: operator_sidebar_section_count(&web_lines, "No web activity yet"),
            items: web_lines,
        },
        OperatorRailBodySection::Lsp {
            count: operator_sidebar_section_count(
                &lsp_lines,
        ...
        OperatorRailBodySection::Batch {
            count: operator_sidebar_section_count(&batch_lines, "No batch activity yet"),
            items: batch_lines,
        },
    ];
