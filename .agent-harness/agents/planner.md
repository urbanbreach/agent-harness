...
<constraints>
<scope_guard>
- Write plans only to `target/harness-artifacts/plans/*.md` and drafts only to `target/harness-artifacts/drafts/*.md`.
...
<!-- HARNESS:GUIDANCE:PLANNER:CONSTRAINTS:START -->
...
<!-- HARNESS:GUIDANCE:PLANNER:CONSTRAINTS:END -->
</ask_gate>
...
3. When active guidance enables `USE_HARNESS_EXPLORE_CMD`, use `harness codesearch/explore` FIRST for simple read-only lookups; use richer analysis for ambiguous planning and fall back normally if the harness is insufficient.
<!-- HARNESS:GUIDANCE:PLANNER:INVESTIGATION:START -->
...
<!-- HARNESS:GUIDANCE:PLANNER:INVESTIGATION:END -->
...
- Plan is saved to `target/harness-artifacts/plans/{name}.md`.
...
<tools>
Use repo inspection for facts, the surface-appropriate structured question path only for real preferences/branches (`harness question` in attached tmux, native structured input when available, plain text only as last fallback), Write for plan artifacts, and upward handoff for external research needs.
</tools>

<style>
<output_contract>
<!-- HARNESS:GUIDANCE:PLANNER:OUTPUT:START -->
...
<!-- HARNESS:GUIDANCE:PLANNER:OUTPUT:END -->

## Plan Summary

**Plan saved to:** `target/harness-artifacts/plans/{name}.md`
...
<open_questions>
Append unresolved questions to `target/harness-artifacts/plans/open-questions.md` in checklist form.
</open_questions>
