...
impl RuntimeExperience {
    ...
    pub fn on_event(&mut self, event: &EventEnvelopeV1) {
        match &event.payload {
            ...
            EventV1::PermissionRequested(_) => {
                self.title_state.set_activity(TitleActivity::AwaitingPermission);
    ...
    fn submit_media(&mut self, text: &str, tick: u64) {
        ...
        if lower.contains("```mermaid") {
            let request_id = self.mermaid_worker.submit(text.to_string(), 0, 80, tick + 30);
    ...
    fn submit_video(&mut self, source: &[u8]) {
        let descriptor = SubprocessDescriptor {
            ...
            args: vec!["-i".to_string(), String::from_utf8_lossy(source).into_owned()],
    ...
    pub fn tick(&mut self, app: &AppState) {
        ...
        self.lifecycle.set_pending_permissions(u8::from(app.active_permission().is_some()));
        self.lifecycle
        ...
        let _ = self.tips.update(&TipContext {
            ...
            viewport_compact: app
                .last_frame_area()
                .is_some_and(|area| area.width < 80),
            model_selected: true,
    ...
    pub fn post_flush<W: Write>(&mut self, out: &mut W) {
        ...
        let elapsed = u64::try_from(self.frame_started_at.elapsed().as_micros()).unwrap_or(u64::MAX);
