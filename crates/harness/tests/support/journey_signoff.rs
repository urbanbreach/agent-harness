...
pub(crate) const JOURNEY_L1_FREEZE_PREFIX: &str =
    "target/test-lanes/latest/signoff-parity/evidence/reference/freeze/journey-";
pub(crate) const JOURNEY_L1_FREEZE_SUFFIX: &str = "-l1-ref-v1/";
pub(crate) const JOURNEY_L4_RECEIPT_PREFIX: &str =
    "target/test-lanes/latest/signoff-parity/evidence/receipts/journey-";
pub(crate) const JOURNEY_L4_RECEIPT_SUFFIX: &str = "-l4-differential-v1.json";
...
/// Assert a journey layer points at its canonical paired artifact path.
///
/// Wave 6 (2026-07-30 signoff-parity clean room): journey rows pair the real
/// L1 reference-CLI freeze and L4 nonvisual differential receipt instead of
/// leaving them empty. The check is structural (canonical prefix/suffix);
/// on-disk provenance is the strict lane's job, not the unit contract's.
fn assert_paired_journey_layer(
    journey_id: &str,
    row: &Value,
    layer: &str,
    prefix: &str,
    ...
    assert!(
        ...
        "{journey_id} evidence_paths.{layer} must be a canonical paired journey artifact ({prefix}...{suffix}), got {value}"
    );
}
...
fn assert_config_journey_evidence(row: &Value, journey_id: &str, l3: &str, l6: &str, strict: bool) {
    // Wave 6 (2026-07-30 signoff-parity clean room): journey rows are promoted
    // to pass once the lane regenerates fresh L3 captures and pairs the real
    // L1 reference-CLI freeze with an L4 nonvisual differential receipt
    // (Contract §4). The structural wiring (L2/L3/L5/L6 owner paths) is still
    // asserted below so a row that loses its ownership trail still fails.
    assert_eq!(
        row["status"].as_str(),
        Some("pass"),
        "{journey_id} must be promoted to pass after the signoff-parity lane regenerates fresh evidence"
    );
    assert_paired_journey_layer(
        journey_id,
        row,
        "L1",
        JOURNEY_L1_FREEZE_PREFIX,
        JOURNEY_L1_FREEZE_SUFFIX,
    );
    assert_eq!(
    ...
    assert_paired_journey_layer(
        ...
        row,
        "L4",
        JOURNEY_L4_RECEIPT_PREFIX,
        JOURNEY_L4_RECEIPT_SUFFIX,
    );
...
pub(crate) fn assert_all_journey_manifest_evidence_with(rows: &[Value], strict: bool) {
    ...
    // Wave 6 (2026-07-30 signoff-parity clean room): worktree journey promoted
    // to pass with fresh L3 captures and a canonical L1/L4 pairing
    // (Contract §4).
    assert_eq!(
        worktree["status"].as_str(),
        Some("pass"),
        "worktree journey must be promoted to pass after the signoff-parity lane regenerates fresh evidence"
    );
    assert_paired_journey_layer(
        journey_id,
        worktree,
