...
pub(super) fn default_app_uses_harness_dark_theme() {
...
pub(super) fn default_harness_dark_survives_color_level_changes() {
    let mut app = AppState::default();
    ...
    assert_eq!(
        app.theme(),
        &Theme::harness_dark().for_color_level(ColorLevel::Basic)
    );
}
...
pub(super) fn arbitrary_viewport_composer_mouse_uses_rendered_geometry() {
    ...
    let viewport = crate::theme_tokens::ViewportId::closest(120, 40);
    assert_eq!(viewport, crate::theme_tokens::ViewportId::Wide132x40);
