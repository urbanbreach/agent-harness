...
fn supported_media_serializes_in_submission_order() {
    ...
    assert_eq!(serialized.iter().map(SerializedAttachment::id).collect::<Vec<_>>(), ["png", "jpeg", "text"]);
    assert_eq!(serialized[0].data_url(), Some("data:image/png;base64,iVBORw=="));
    assert_eq!(serialized[1].data_url(), Some("data:image/jpeg;base64,/9j/"));
