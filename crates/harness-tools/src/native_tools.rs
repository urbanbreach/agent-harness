...
impl Tool for BackgroundOutputTool {
    tool_metadata!(
        "background_output",
        "Provides durable retrieval of the current status or terminal result for a child task scheduled with task(run_in_background=true). Use block=false to check status or block=true to wait for completion and retrieve the full result. Wait expiry does not cancel children. Prefer request_id from the task result; task_id/session_id resolve to the latest child request for compatibility. Set cancel=true anytime to request coordinator cancellation for a non-terminal child task. For multi-wait, pass request_ids with wait_mode=`any` (first terminal) or `all` (every terminal) and block=true.",
        ToolCapability::SpawnAgent,
