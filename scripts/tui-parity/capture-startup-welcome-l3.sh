...
# welcome panel settles with bordered actions + the two-line clipboard warning + empty
# bordered composer strip).
#
# Output:
...
if ! grep -q 'Clipboard may be unreachable.' "$EVIDENCE_DIR/terminal.txt"; then
  echo "FAIL: welcome L3 missing the reference clipboard warning band" >&2
  exit 1
fi
if ! grep -q '❯' "$EVIDENCE_DIR/terminal.txt"; then
...
if ! grep -q 'Logged in with API key' "$EVIDENCE_DIR/terminal.txt"; then
