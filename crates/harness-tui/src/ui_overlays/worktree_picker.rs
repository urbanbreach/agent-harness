...
pub(super) fn render_worktree_picker_overlay(
    frame: &mut Frame,
    app: &AppState,
    theme: &Theme,
    root: Rect,
    ...
    let overlay_width = root.width.clamp(44, 88).min(root.width);
    ...
    if let Some(error) = app.worktree_picker.error.as_deref() {
        if list_height > 0 {
    ...
    let entries = &app.worktree_picker.entries;
    if entries.is_empty() {
        if list_height > 0 {
            let area = Rect::new(inner.x, list_y, inner.width, 1);
            frame.render_widget(
                Paragraph::new(Line::from(Span::styled("No worktrees found", muted_style))),
                area,
            );
        }
        return;
    ...
    for (visible_index, row_index) in (scroll..entries.len()).take(visible_rows).enumerate() {
        ...
        let is_selected = row_index == app.worktree_picker.selected;
