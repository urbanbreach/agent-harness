...
fn production_app_state_routes_keyboard_input_through_atom_composer() {
    ...
    let submission = app.composer_submission().expect("valid production composer submission");
...
fn production_app_state_routes_completion_and_queue_actions() {
    ...
    assert_eq!(app.composer_view_model(ViewportId::Standard100x30).text, "status");
