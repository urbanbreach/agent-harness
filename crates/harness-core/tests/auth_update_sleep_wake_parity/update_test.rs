...
fn verified_update_pipeline_rejects_bad_hash_and_rolls_back_interruption() {
    // arrange — a local update with a known artifact checksum and an existing binary.
    ...
    // act — update availability, verified download, apply, mismatch, and interruption paths run.
    ...
    // assert — verified update applies, a bad hash is removed, rollback restores the prior binary, and restart is explicit.
