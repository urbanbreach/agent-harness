...
//!
//! Compatibility cut planning over the legacy event model. Walks backward
//! through an agent's events, accumulating token estimates, and finds the first
//! valid cut point that keeps approximately `keep_recent_tokens`.

use crate::event::{EventEnvelopeV1, EventV1};
use crate::session::AssistantPart;

use super::super::tokens::estimate_text_tokens;
