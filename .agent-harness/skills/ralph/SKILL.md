...
- Integrate with external goal context when goal tools are available: inspect the active thread goal with `get_goal`, preserve it as the top-level stop condition, and only call `update_goal({status: "complete"})` after a Ralph completion audit proves the objective is actually achieved.
</Execution_Policy>
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Record the verdict as harness workflow visual evidence (artifact JSON plus numeric + qualitative feedback) so replay/status can project it.
...
6. **Verify completion with fresh evidence**:
   - If external goal context is available, call `get_goal` before final verification to restate the active objective and include it in the evidence checklist.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `Agent Harness:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
8. **On approval**: If external goal context is active, call `update_goal({status: "complete"})` only after the harness completion audit passes. Then use `$cancel` or harness workflow signoff/cancel surfaces for clean exit; do not delete state files manually.
...
- Use harness workflow evidence, continuation reminders, and projected workflow status for Ralph state between iterations; do not write external mode-state files.
...
- Persist context snapshot paths as harness workflow evidence/artifact refs so later phases and agents share the same grounding context.
- If a legacy Harness workflow state instruction appears in older artifacts, treat it as upstream reference material only and translate it to harness workflow evidence/status operations.
</Tool_Usage>

## Goal Mode Integration

external goal context is the thread-level completion contract for long-running Ralph work. Ralph state tracks workflow mechanics; goal mode tracks whether the user objective is truly done. When the goal tools are available:
...
Harness records Ralph lifecycle, phase, continuation iterations, context snapshots, verification, and closeout through coordinator-owned events and workflow projections. Do **not** run `harness workflow evidence` or write per-mode `Harness workflow projection/*.json` files inside the harness.

- **On start**: `$ralph` dispatch records a workflow start event, starts a workflow-owned continuation, and submits the Ralph protocol prompt.
- **On each iteration**: continuation reminder events and the active workflow context overlay carry iteration/phase guidance back into the prompt.
- **On verification/fix transitions**: record verification, cleanup, and architect-review evidence with workflow evidence/tool artifacts.
- **On completion**: only complete/sign off when the completion audit, required tests, cleanup, and architect approval are all backed by fresh evidence.
- **On cancellation/cleanup**: use `$cancel` or harness workflow cancel/signoff surfaces; never clean up by deleting `Harness workflow projection` files.
...
5. Initialize progress tracking through harness workflow evidence/projection entries
