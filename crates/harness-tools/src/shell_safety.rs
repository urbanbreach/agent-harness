    ...
    for token in &command.args {
        let candidate = if token.starts_with('-') {
            if let Some((_, value)) = token.split_once('=') {
                ...
                chars.next();
                chars.next();
                let offset = token.len() - chars.as_str().len();
                &token[offset..]
            } else {
                continue;
            }
...
mod tests {
    ...
    fn validate_bash_command_rejects_external_path_arguments() {
        ...
        let err5 = safety
            ...
            .expect_err("external absolute path inside short flag should be blocked");
        ...
        let err6 = safety
            ...
            .expect_err("external relative path inside short flag should be blocked");
