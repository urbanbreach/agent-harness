...
- **After Pass 1 completes**: Write extraction summary to `Harness workflow projection/{scope}/web-clone-extraction.json` containing:
...
- **After each Pass 4 verification**: Append the composite verdict to `Harness workflow projection/{scope}/web-clone-verdicts.json`.
- **When running within ralph**: Also persist the `visual` portion of the composite verdict to `Harness workflow projection/{scope}/ralph-progress.json` for ralph compatibility, mapping `visual.score` → top-level `score` and `visual.verdict` → top-level `verdict`.
