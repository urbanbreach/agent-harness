...
fn adjacent_segments(key: &str, left: &str, right: &str) -> bool {
    let mut prev_matched_left = false;
    for segment in key
        .split(|c: char| !c.is_ascii_alphanumeric())
        .filter(|s| !s.is_empty())
    {
        if prev_matched_left && segment.eq_ignore_ascii_case(right) {
            return true;
        }
        prev_matched_left = segment.eq_ignore_ascii_case(left);
    }
    false
}
