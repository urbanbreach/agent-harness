...
async fn file_discovered_hook_blocks_without_trust_then_runs_as_a_local_process() {
    // Given: a filesystem-local package with a discovered hooks file.
    ...
    // When: trust is denied, then explicitly granted before lifecycle execution.
    registry
    ...
    // Then: discovered components activate, and the allowed hook has a real process effect.
...
fn local_plugin_source_survives_reload_disable_enable_and_uninstall() {
    // Given: a durable registry and a local directory source with hooks and skills.
    let temp = tempfile::tempdir().unwrap_or_abort();
    let workspace = temp.path().join("workspace");
    fs::create_dir_all(&workspace).unwrap_or_abort();
    let package = write_local_plugin(&workspace, "local.source.plugin");

    // When: the local source is installed, trusted, and re-opened after restart.
    let mut registry = PluginLifecycleRegistry::open(&workspace).unwrap_or_abort();
    registry
        .install_from_package_root(&package)
        .unwrap_or_abort();
    ...
    // Then: persisted activation is observable and disable/enable/uninstall survive restarts.
...
fn local_acp_stdio_and_file_agent_modes_restart_with_observable_effects() {
    // Given: local stdio and file-backed ACP transports.
    let temp = tempfile::tempdir().unwrap_or_abort();

    // When: stdio agent mode is started twice and the file agent mode writes a frame.
    ...
    // Then: each local transport completes without a network endpoint.
...
fn configured_local_mcp_and_lsp_surfaces_preserve_stdio_http_and_consent_shapes() {
    // Given: local stdio MCP, configured HTTP MCP, and a local LSP command.
    ...
    // When: local-only transport and consent configuration is inspected.
    let serialized = serde_json::to_string(&lsp).unwrap_or_abort();

    // Then: configured endpoints stay explicit; no implicit network provisioning is introduced.
...
fn persistent_code_graph_returns_real_relationships_from_local_filesystem() {
    // Given: a local source tree where beta calls alpha.
    let temp = tempfile::tempdir().unwrap_or_abort();
    ...
    // When: a persistent index is built and its definition/reference relationships are queried.
    ...
    // Then: the graph is persistent and relationship-aware.
...
async fn replay_of_local_integration_session_is_side_effect_free() {
    // Given: a persisted event log next to an already-created local plugin receipt.
    let temp = tempfile::tempdir().unwrap_or_abort();
    ...
    // When: replay reads the session twice.
    for _ in 0..2 {
    ...
    // Then: replay did not execute integrations or mutate the durable event log.
