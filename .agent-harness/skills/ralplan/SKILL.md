...
2. **User feedback** *(--interactive only)*: If `--interactive` is set, use the structured question UI (`native question tool` in attached native terminal UI; native structured input outside native terminal UI when available) to present the draft plan **plus the Principles / Drivers / Options summary** before review (Proceed to review / Request changes / Skip review). Otherwise, automatically proceed to review.
...
- `$ultragoal` — **default goal-mode follow-up** for implementation or general goal-oriented follow-up plans that should become durable Codex/Harness goals with sequential completion tracking.
