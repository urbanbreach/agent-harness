...
pub(super) async fn denied_tool_turn(
    ...
    let request_id = coordinator
        ...
        .await
        .unwrap_or_abort();
    let mut request_events = events
        .map(|event| event.unwrap_or_abort())
        .filter(|event| event.correlation_id.as_deref() == Some(request_id.as_str()));
    let deny_permission = async |event: &EventEnvelopeV1| {
    ...
    tokio::time::timeout(Duration::from_secs(2), async {
        ...
        while !completed {
            let event = request_events.next().await.unwrap_or_abort();
            deny_permission(&event).await;
            completed = matches!(
                event.payload,
                EventV1::TaskCompleted(_) | EventV1::TaskCancelled(_)
            );
