...
This document tracks the active Harness-to-Harness transcript rendering comparison.
...
1. Happy path: a deterministic transcript with user text, assistant markdown,
   reasoning, and tool rows renders with Harness spacing and entry semantics.
2. Edge: long markdown/code/tool text wraps without terminal overflow at narrow
   widths.
3. Adjacent regression: task/subagent activity renders as Harness task rows, with
   subagent details routed through the subagent surface instead of extra transcript
   hint rows.

## Findings

### Harness entry semantics

- Harness's immutable scrollback maps committed user entries to plain text with
  a leading `› ` prompt marker in
  `inspirations/packages/src/cli/cmd/run/entry.body.ts:51`,
  colors that text with `entry.user.body = scrollbackTheme.primary` in
  `inspirations/packages/src/cli/cmd/run/theme.ts:545`, and
  writes it through `RunEntryContent` as a full-width wrapped text renderable in
  `inspirations/packages/src/cli/cmd/run/scrollback.writer.tsx:151`.
- Harness's active sent-message/prompt surface is a separate footer surface: it
  uses `border={["left"]}`, `borderColor={theme().highlight}`, and a custom
  left border glyph `vertical: "█"` in
  `inspirations/packages/src/cli/cmd/run/footer.view.tsx:642`;
  the inner prompt container uses `paddingLeft={0}`, `paddingRight={0}`,
  `paddingTop={0}`, `gap={0}`, and `backgroundColor={theme().surface}` in
  `inspirations/packages/src/cli/cmd/run/footer.view.tsx:656`;
  the prompt body itself adds `paddingTop={1}`, `paddingBottom={1}`, and
  `paddingRight={2}` around the textarea in
  `inspirations/packages/src/cli/cmd/run/footer.prompt.tsx:252`.
- Harness maps reasoning to markdown code content and rewrites leading
  `Thinking:` to `_Thinking:_` in
  `inspirations/packages/src/cli/cmd/run/entry.body.ts:61`.
- Harness removes `[REDACTED]` from reasoning entries and suppresses the entry
  when the cleaned body is empty in
  `inspirations/packages/src/cli/cmd/run/entry.body.ts:61`.
- Harness suppresses assistant start/final text and renders in-flight assistant
  text as markdown in `inspirations/packages/src/cli/cmd/run/entry.body.ts:180`.
- Harness keeps inline tool rows adjacent without a blank separator; block rows
  get one separator row in
  `inspirations/packages/src/cli/cmd/run/scrollback.writer.tsx:81`.
- Harness classifies tool text rows as `inline`, while assistant text is `block`;
  ...
  separator row in
  `inspirations/packages/src/cli/cmd/run/scrollback.writer.tsx:52`.

### Harness task/subagent behavior

- Harness task inline rows use status icons `✗`, `•`, and `✓`, with title equal
  to the task description when present and subtitle `${Kind} Agent` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:366`.
- Harness final task snapshots are structured blocks titled `# ${Kind} Task`
  with the task description as the body row in
  `inspirations/packages/src/cli/cmd/run/tool.ts:571`.
- Harness completed `task` tool output extracts `<task_result>...</task_result>`
  or drops `task_id:` metadata lines, then renders the remaining result through
  markdown in `inspirations/packages/src/cli/cmd/run/tool.ts:759`
  and `inspirations/packages/src/cli/cmd/run/tool.ts:1436`.
- Harness subagent inspection lives in the footer panel, not as a transcript hint:
  `RunFooterSubagentBody` renders the selected subagent title/status and commits
  under a scrollbox in
  `inspirations/packages/src/cli/cmd/run/footer.subagent.tsx:121`.
- Harness subagent inspection keeps the selected child activity scrolled to the
  newest rows by setting `stickyScroll={true}` and `stickyStart="bottom"` on the
  footer scrollbox in
  `inspirations/packages/src/cli/cmd/run/footer.subagent.tsx:150`.
- Harness reserves 14 rows for the subagent inspector in
  `inspirations/packages/src/cli/cmd/run/footer.subagent.tsx:11`
  and uses status icons `●`, `○`, `◍`, and `◔` in
  `inspirations/packages/src/cli/cmd/run/footer.subagent.tsx:29`.

### Harness todo behavior

- Harness todo-write inline/block rows are titled `Todos`, use icon `#`, and
  render todo item markers as `[✓]` only for `completed`, `[•]` for
  `in_progress`, and `[ ]` for every other status in
  `inspirations/packages/src/cli/cmd/run/tool.ts:377`.
- Harness structured scrollback renders cancelled items as struck `[ ]` rows in
  `inspirations/packages/src/cli/cmd/run/scrollback.writer.tsx:10`.

### Harness skill behavior

- Harness skill rows use icon `→` and title `Skill "${name}"` with no output
  body in `inspirations/packages/src/cli/cmd/run/tool.ts:396`.

### Harness LSP behavior

- Harness LSP rows use icon `→` and title `LSP ${operation}` or
  `LSP ${operation} ${filePath}:${line}:${character}` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:445`.

### Harness shell and file-search behavior

- Harness bash rows render through the run-mode `$` block helper: the row title
  is the command itself and the completed output is printed as block body in
  `inspirations/packages/src/cli/cmd/run/tool.ts:1272` and
  `inspirations/packages/src/cli/cmd/run.ts:78`, with no
  Harness-only rail/card chrome.
- Harness grep rows use icon `✱`, title `Grep "${pattern}"`, and optional
  description `in ${path} · N matches` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:301`.
- Harness glob rows use icon `✱`, title `Glob "${pattern}"`, and optional
  description `in ${path} · N matches` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:286`.
- Harness list rows use icon `→` and title `List ${path}` or `List`, without a
  count/path subtitle, in
  `inspirations/packages/src/cli/cmd/run/tool.ts:314`.

### Harness batch/write/edit/patch behavior

- Harness batch rows use icon `#`, block mode, and title `Batch N tool(s)` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:435`.
- Harness write and edit rows use icon `←`, block mode, and titles
  `Write ${filePath}` / `Edit ${filePath}` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:332` and
  `inspirations/packages/src/cli/cmd/run/tool.ts:349`.
- Harness apply-patch rows use icon `%` and title `Patch`, `Patch 1 file`, or
  `Patch N files` in
  `inspirations/packages/src/cli/cmd/run/tool.ts:403`.

### Harness markdown table behavior

- Harness streams assistant markdown with `MarkdownRenderable` using
  `internalBlockMode: "top-level"` and `tableOptions: { widthMode: "content" }`
  in `inspirations/packages/src/cli/cmd/run/scrollback.surface.ts:175`.
- Harness commits stable markdown by top-level block rather than by raw line in
  `inspirations/packages/src/cli/cmd/run/scrollback.surface.ts:287`,
  which keeps table blocks visually distinct from surrounding paragraph text.

### Harness fenced code behavior

- Harness renders standalone reasoning/code bodies through `CodeRenderable` with
  `width: "100%"`, `wrapMode: "word"`, `drawUnstyledText: false`, streaming
  enabled, and theme-derived syntax style in
  `inspirations/packages/src/cli/cmd/run/scrollback.surface.ts:164`.
- Harness renders markdown-contained fenced code through `MarkdownRenderable`
  with the same theme syntax style and top-level block streaming in
  `inspirations/packages/src/cli/cmd/run/scrollback.surface.ts:175`.

### Harness parity behavior implemented

- Harness now applies Harness user-entry and reasoning-entry body semantics in
  ...
  `inspirations/packages/src/cli/cmd/run/entry.body.ts:51`
  and `inspirations/packages/src/cli/cmd/run/entry.body.ts:61`.
- Harness top-level user rows follow the known-good GitHub `origin/dev`
  Harness-style transcript block rather than the downgraded active-prompt
...
- Harness now suppresses redacted-only reasoning before transcript section
  ...
  bare `Thinking:` rows when Harness would return `RUN_ENTRY_NONE`.
- Harness now renders completed task results as markdown detail blocks in
  ...
  Harness `taskResult` plus final `markdownBody` behavior.
...
- Harness task rows no longer add transcript-visible child toolcount,
  ...
  available through the native background tools instead of the Harness-shaped
  transcript task row.
- Harness task rows render the subtitle through `TaskInline` in
  `crates/harness-tui/src/ui_transcript_tool_render.rs`, keeping the Harness
...
- Harness todo rows now use the fixed `Todos` block title with `#` icon and render cancelled
  ...
  `crates/harness-tui/src/ui_tool_question_todo.rs`, matching the Harness
...
- Harness now lets `transcript_surface_leading_gap` insert a one-row separator
  ...
  `crates/harness-tui/src/ui_transcript_surface.rs`, matching Harness's
...
- Harness now renders markdown table blocks through a focused content-width,
  ...
  from `crates/harness-tui/src/ui_markdown.rs:278`, matching Harness's
...
- Harness no longer renders compatibility-alias disclosure rows such as
  `Compat alias · read → fs.read` in transcript tool details. Harness read and
  webfetch rows render only the operator-facing tool row in
  `inspirations/packages/src/cli/cmd/run/tool.ts:322` and
  `inspirations/packages/src/cli/cmd/run/tool.ts:341`; Harness
  now keeps alias resolution internal in
  `crates/harness-tui/src/ui_transcript_tool_sections.rs` instead of appending a
  non-Harness metadata line.
- Harness no longer renders the read-specific `↳ Loaded ...` detail row after a
  successful read. Harness `runRead` returns only the inline `Read ${file}` row
  and optional input description in
  `inspirations/packages/src/cli/cmd/run/tool.ts:322`, so
  Harness keeps read output details out of the transcript row unless Harness has
  a matching visible field.
- Harness no longer adds transcript-local `Click to expand` / `Click to collapse`
  preview rows or ellipsis truncation for generic and bash tool bodies. Harness
  run-mode block rendering prints the `ToolInline.body` it receives in
  `inspirations/packages/src/cli/cmd/run.ts:78`, and bash sets
  that body from completed output in
  `inspirations/packages/src/cli/cmd/run/tool.ts:1272`; runtime
  truncation remains a tool-output concern rather than a transcript collapse
  affordance.
- Harness bash rows now render as an Harness-style `$ command` block with only
...
- Harness skill rows now render `→ Skill "name"` without the previous
  ...
  `crates/harness-tui/src/ui_transcript_tool_sections.rs`, matching Harness
  `runSkill`.
...
- Harness question rows now render `→ Asked N question(s)` without the previous
  ...
  and `crates/harness-tui/src/ui_transcript_tool_sections.rs`, matching
  Harness `runQuestion`.
- Harness batch, write, edit, and apply-patch rows now use Harness titles and
...
- Harness subagent replay now reserves the Harness 14-row footer inspector in
  ...
  `crates/harness-tui/src/ui_subagent_footer.rs:31`, with Harness-style
...
- Harness subagent footer status now maps child orchestration states to the
  Harness status set in `crates/harness-tui/src/ui_subagent_footer.rs`: running
...
- Harness subagent footer activity now uses
  `crates/harness-tui/src/ui_transcript.rs:612` to render only Harness-style
  ...
  rows. This matches Harness
  `inspirations/packages/src/cli/cmd/run/footer.subagent.tsx:84`
...
- Harness subagent footer overflow now renders the latest visible child rows by
  ...
  `crates/harness-tui/src/ui_subagent_footer.rs:211`, matching Harness's
...
- `transcript_user_and_reasoning_match_reference_entry_body` locks the top-level
  ...
  two-column body padding while preserving Harness's no-duplicate
  `Thinking: Thinking:` reasoning behavior.
- `redacted_only_reasoning_matches_reference_empty_body` locks Harness's
...
- `transcript_task_rows_show_child_status_duration_and_counts` locks completed
  task rows to the Harness inline title/subtitle shape without child
...
- `subagent_footer_body_renders_child_user_commit_without_assistant_summary`
  locks the selected child footer body to Harness `RunEntryContent` semantics:
...
- `file_search_rows_match_reference_title_description_shape` locks grep, glob,
  and list rows to the Harness title/subtitle shape: `✱ Grep "fn render"` with
...
- `native_tool_transcript_rows_show_reference_timestamps_and_task_metadata`
  locks current Harness web rows from `run/tool.ts`: `% WebFetch <url>`,
...
- Fresh body→tool spacing evidence lives under
  `/tmp/harness/harness-tui-spacing-parity-qa/`, with the direct rendered
  separator capture in `body-tool-spacing.txt`.
- Fresh markdown table parity evidence lives under
  `/tmp/harness/harness-tui-markdown-table-parity-qa/`, with direct rendered
...
- Fresh todo parity test and TUI-width evidence lives under
  `/tmp/harness/harness-tui-todo-parity-qa/`, with the direct rendered todo
  rows captured in `rendered-todo.txt`.
- Fresh tmux capture evidence lives under
  `/tmp/harness/harness-tui-task-row-qa/` for the 180-column deterministic TUI
  surface.
- Fresh fenced-code replay evidence lives under
  `/tmp/harness/harness-tui-code-block-parity-qa/`: `live-code-tui-capture.txt`
...
- Fresh narrow wrapping/cutoff evidence lives under
  `/tmp/harness/harness-tui-wrapping-parity-qa/`: the pre-fix captures showed
...
- Fresh `harness.jsonc` dogfood evidence also lives under
  `/tmp/harness/harness-tui-wrapping-parity-qa/`: `harness-jsonc-dogfood-capture.txt`
...
- Fresh final parity replay evidence lives under
  `/tmp/harness/harness-tui-final-parity-qa/`: `replay-session/events.jsonl`
...
- Fresh continuation dogfood evidence also lives under
  `/tmp/harness/harness-tui-final-parity-qa/`: `continue-dogfood-capture.txt`
...
- Fresh output-collapse replay evidence lives under
  `/tmp/harness/harness-tui-output-collapse-qa/`: `replay-capture.txt`
...
- Fresh skill-row replay evidence lives under
  `/tmp/harness/harness-tui-skill-row-qa/`: `replay-capture.txt` exercises the
...
- Fresh LSP-row replay evidence lives under
  `/tmp/harness/harness-tui-lsp-row-qa/`: `replay-capture.txt` exercises the
...
- Fresh question-row replay evidence lives under
  `/tmp/harness/harness-tui-question-row-qa/`: `replay-capture.txt` exercises
...
- Fresh batch/write/edit/patch replay evidence lives under
  `/tmp/harness/harness-tui-batch-write-patch-qa/`: `replay-capture.txt`
...
- Fresh subagent-footer replay evidence lives under
  `/tmp/harness/harness-tui-subagent-footer-qa/`: the current capture
  ...
  surface at `80x24` and shows the selected Harness-style padded footer body
...
- `child-replay-capture-current.txt` in
  `/tmp/harness/harness-tui-subagent-sticky-qa/` exercises the current
...
- `subagent_footer_status_uses_running_and_cancelled_icons` locks the Harness
  footer status set for active and cancelled child orchestration rows: the first
  spinner frame `⠋` for running and `○` for cancelled.
- `subagent_footer_body_sticks_to_latest_activity` locks Harness's
...
- `subagent_footer_body_preserves_child_text_matching_parent_label` locks that
  ...
  happens to contain the parent label remains intact, matching Harness's lack
  of an equivalent content rewrite.
- Fresh sticky-bottom subagent replay evidence lives under
  `/tmp/harness/harness-tui-subagent-sticky-qa/`:
...
- Fresh subagent-footer tool-row replay evidence lives under
  `/tmp/harness/harness-tui-subagent-toolrow-qa/`:
...
- Redacted `harness.jsonc` dogfood checklist evidence is tracked at
  `/tmp/harness/harness-tui-subagent-footer-qa/redacted-dogfood-checklist.md`:
  it records the exhaustive chat-rendering parity matrix for every
  `docs/native-tool-catalog.md` tool id, the exact Harness-vs-Harness
  ...
  artifacts in the same directory are `same-transcript-harness-harness-diff.txt`
...
- No known transcript/tool-row blocker remains in the parity slices documented
  ...
  original broad Harness-to-Harness chat-rendering parity goal.
