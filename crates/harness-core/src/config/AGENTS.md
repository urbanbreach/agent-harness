...
## OVERVIEW
Runtime/TUI config implementation: public contract, compatibility aliases, discovery, validation, settings registry/write, model/provider registries, skills/LSP/MCP/hook settings, and schema generation.
...
4. `aliases.rs`, `provider.rs`, `public/normalization.rs`, and `public.rs` merge compatibility aliases into canonical fields with conflict detection.
...
7. `settings_registry.rs`/`settings_write.rs` expose the typed public settings registry and the narrow runtime-only project-file write path (secrets fail closed).

## WHERE TO LOOK
| Task | Location | Notes |
|------|----------|-------|
| Public contract | `public/contract.rs`, `public.rs`, `public/normalization.rs` | Canonical/compatibility/inert/unsupported key status, normalization, docs/schema metadata. |
...
| Providers/models | `provider.rs`, `model_types.rs`, `model_catalog.rs`, `model_selection.rs`, `model_alias.rs` | Provider options, variants, model profile fallback metadata, alias resolution. |
| Integrations | `integrations.rs`, `validation.rs` | MCP/remote search/LSP/hook/skill validation. |
| Settings registry/write | `settings_registry.rs`, `settings_write.rs` | Typed setting definitions, sensitivity/scope metadata, runtime-only project-file write-back. |
| Defaults/registries | `defaults.rs`, `registries.rs` | Default values and global read-only runtime snapshots. |
| Tests | `tests.rs`, `tests/` | Discovery merge, public basics, agents, permissions/models, env assets, formatter, settings. |
...
- `settings_write.rs` writes only registered runtime-surface settings to `harness.json{,c}`; secret and TUI-surface settings fail closed.
