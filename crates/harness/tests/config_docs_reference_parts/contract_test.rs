...
pub use harness_providers::ProviderErrorCategory;

pub use harness_tui::{Action, KeyMap};

pub use crate::common::repo_root;
...
#[test]
fn config_docs_tui_default_keybindings_match_keymap_defaults() {
    let doc = read_doc("docs/config.md");
    let documented = documented_tui_default_bindings(&doc);
    let keymap = KeyMap::with_defaults();
    let expected_actions = [
        ("palette", Action::Palette),
        ("new_session", Action::NewSession),
        ("resume_session", Action::ResumeSession),
        ("switch_model", Action::SwitchModel),
        ("open_status_dialog", Action::OpenStatusDialog),
        ("open_lineage_browser", Action::OpenLineageBrowser),
        ("compact_session", Action::CompactSession),
        ("help", Action::Help),
        ("quit", Action::Quit),
        ("agent_cycle", Action::AgentCycle),
        ("agent_cycle_reverse", Action::AgentCycleReverse),
        ("focus_next", Action::FocusNext),
        ("focus_prev", Action::FocusPrev),
        ("toggle_operator_sidebar", Action::ToggleOperatorSidebar),
        ("toggle_terminal_panel", Action::ToggleTerminalPanel),
        ("toggle_follow", Action::ToggleFollow),
        ("reload", Action::Reload),
        ("close_review_surface", Action::CloseReviewSurface),
        ("open_event_log", Action::OpenEventLog),
        ("session_child_first", Action::SessionChildFirst),
        ("session_child_cycle", Action::SessionChildCycle),
        (
            "session_child_cycle_reverse",
            Action::SessionChildCycleReverse,
        ),
        ("session_parent", Action::SessionParent),
        ("diff_hunk_next", Action::DiffHunkNext),
        ("diff_hunk_previous", Action::DiffHunkPrevious),
        ("move_down", Action::MoveDown),
        ("move_up", Action::MoveUp),
        ("submit_prompt", Action::SubmitPrompt),
        ("insert_newline", Action::InsertNewline),
        ("clear_prompt", Action::ClearPrompt),
        ...
        ("allow_permission", Action::AllowPermission),
        ("deny_permission", Action::DenyPermission),
        ("dismiss_modal", Action::DismissModal),
        ("variant_cycle", Action::VariantCycle),
    ];

    let documented_actions = documented.keys().cloned().collect::<BTreeSet<_>>();
    let expected_action_ids = expected_actions
        .iter()
        .map(|(action, _)| (*action).to_string())
        .collect::<BTreeSet<_>>();

    assert_eq!(
        documented_actions, expected_action_ids,
        "tui default binding table drifted from keymap contract"
    );

    for (action_id, action) in expected_actions {
        let expected_binding = keymap.get_binding_str(action);
        assert_eq!(
            documented.get(action_id),
            Some(&expected_binding),
            "{action_id} default binding drifted"
        );
    }
}
