...
**Status:** planned V1 implementation slice after `docs/v1-first-slice-prd.md`.  
...
- [ ] Add or update a focused implementation checklist in the PR or checkpoint notes, not in generated artifacts.
- [ ] Add failing tests for agent catalog resolution covering primary, subagent, hidden, and category routes.
- [ ] Add failing tests for native tool catalog parity covering all currently registered native tool ids.
- [ ] Add failing tests for model-visible session tools using deterministic fixture sessions.
- [ ] Add failing tests for `background_cancel` equivalence with `background_output(cancel=true)`.
- [ ] Add failing tests for narrow `team_list` projection behavior.
- [ ] Add failing tests for `ast_grep_search` schema and workspace safety.
- [ ] Add conditional failing tests for `ast_grep_replace` only if implementation is attempted.
- [ ] Add docs drift tests or assertions for any public docs modified by this slice.

### 4.2 Phase 2: Agent Catalog Seam

- [ ] Create a thin catalog seam in `harness-core` or the narrowest existing config/projection owner that can be reused by CLI, tools, doctor, support export, and TUI without moving coordinator authority.
- [ ] The catalog resolves shipped primary profiles: `build`, `plan`, `discipline`.
- [ ] The catalog resolves shipped subagents: `explore`, `general`.
- [ ] The catalog resolves shipped category routes: `visual-engineering`, `artistry`, `ultrabrain`, `deep`, `quick`, `unspecified-low`, `unspecified-high`, `writing`.
- [ ] The catalog resolves hidden profiles such as title, summary, and compaction when present.
- [ ] Each entry includes stable id, display name, role, mode, hidden flag, category binding, display order, prompt asset status, prompt source, model ref, resolved provider/model/variant where available, fallback chain, toolset, permission posture, skill metadata, and readiness warnings.
- [ ] Category fallback policy is represented once and reused by task output and doctor.
- [ ] Unknown category fallback remains visible and follows existing documented policy.
- [ ] Plan-to-Explore restriction remains runtime-enforced and tested.
- [ ] Read-only subagent restrictions remain runtime-enforced and tested.
- [ ] The catalog does not add specialist personas or aliases beyond existing shipped profiles.
- [ ] No event schema change is added unless unavoidable; if unavoidable, update `docs/architecture.md` and event drift tests in the same phase.

### 4.3 Phase 3: Native Tool Catalog Seam

- [ ] Add a native tool catalog seam owned by `harness-tools` or `harness-core::tool`, not by docs or TUI.
- [ ] Each tool entry includes canonical id, provider function name if different, aliases if any, description summary, capability, permission kind, actor availability, supervisor-only status, schema status, mutation/read-only status, replay behavior, artifact behavior, and docs status.
- [ ] Existing canonical ids remain stable.
- [ ] Worker registry filtering remains through `ActorKind::Worker`.
- [ ] MCP generic wrappers remain available and are clearly separated from built-in native tools.
- [ ] Doctor can report the tool catalog without network calls or MCP server startup.
- [ ] Tool parity tests fail if a registered required V1 tool is missing from the catalog or if docs list an unregistered tool.
- [ ] Permission docs map each tool to canonical permission names: `bash`, `edit`, `question`, `task`, `webfetch`, `websearch`, `codesearch`, and `lsp`.

### 4.4 Phase 4: Model-Visible Session Tools

- [ ] Add `session_list` as a native model-visible tool.
- [ ] Add `session_read` as a native model-visible tool.
- [ ] Add `session_search` as a native model-visible tool.
- [ ] Add `session_info` as a native model-visible tool.
- [ ] Reuse `crates/harness/src/sessions.rs`, `harness_core::proj`, `harness_core::session_lineage`, and replay projections by extracting reusable core/library helpers where needed.
- [ ] Do not shell out to the CLI from native tools.
- [ ] Do not execute providers, tools, hooks, MCP servers, or network calls while reading sessions.
- [ ] Outputs are capped, redacted, structured JSON plus concise display text.
- [ ] Tools reject traversal and out-of-session-root access.
- [ ] Tools expose enough metadata for run id, title, status, profile, provider/model, mode source, resumability, artifact count, child count, parent id, timestamps, and event counts where available.
- [ ] `session_read` supports bounded reading by run id/path selector, message/event window, and redaction-on by default.
- [ ] `session_search` supports text search across safe replay-derived message/tool summaries and returns capped excerpts with session/run ids.
- [ ] `session_info` reports metadata, lineage, status, event counts, artifact index summary, and recovery/resume notes without dumping whole logs.
- [ ] Session tool output explicitly states `source: "event_replay"` or equivalent.

### 4.5 Phase 5: Dedicated Background Cancel

- [ ] Add `background_cancel` as a first-class native tool.
- [ ] `background_cancel` uses the existing coordinator cancellation path used by `background_output(cancel=true)`.
- [ ] Required selector is `request_id` unless compatibility requires `task_id` or `session_id`; ambiguous selectors fail with actionable errors.
- [ ] Optional `reason` is redacted/capped before persistence.
- [ ] Authorization behavior matches current background projection rules.
- [ ] Terminal tasks report no cancellation performed.
- [ ] Non-terminal authorized tasks produce the same lifecycle events as `background_output(cancel=true)`.
- [ ] Late result handling remains `TaskResultLate` with side effects discarded.
- [ ] `background_output(cancel=true)` remains supported and documented as compatibility.
- [ ] Task result `next_actions` prefer `background_cancel(request_id=...)` for cancellation while still listing `background_output` for status/result retrieval.

### 4.6 Phase 6: Mandatory Narrow `team_list`

- [ ] Add `team_list` as mandatory for this slice, but only as a primitive projection-reader over existing active event-sourced team state.
- [ ] `team_list` must not add declared team registries.
- [ ] `team_list` must not add worktrees.
- [ ] `team_list` must not add tmux visualization.
- [ ] `team_list` must not add mailbox artifacts.
- [ ] `team_list` must not add team file claims.
- [ ] `team_list` must not spawn, resume, shut down, or mutate teams.
- [ ] Output includes team run id, name, description, status, lead summary, member counts, task counts, message counts, bounds consumption, created/last monotonic timestamps where projected, and deletion/shutdown state.
- [ ] `team_status` remains the detailed per-team inspection tool.
- [ ] Doctor reports active team count from projections if available, without treating full Team Mode as V1 scope.

### 4.7 Phase 7: AST-Grep Search And Conditional Replace

- [ ] Add `ast_grep_search` as a required native tool after the native tool catalog and parity harness are in place.
- [ ] `ast_grep_search` supports explicit language, pattern, paths, include/exclude globs, context, and output cap.
- [ ] `ast_grep_search` is read-only and maps to the `codesearch` permission kind unless a new permission kind is explicitly justified, documented, and tested.
- [ ] `ast_grep_search` rejects traversal and out-of-workspace paths.
- [ ] `ast_grep_search` returns structured matches with file path, range, language, matched text or capped snippet, and artifact refs when output is large.
- [ ] `ast_grep_search` failure modes are actionable for missing binary/adapter, parse failure, unsupported language, no matches, too many matches, and invalid pattern.
- [ ] `ast_grep_replace` is conditional/stretch, not required for final DoD if safety gates cannot pass.
- [ ] Attempt `ast_grep_replace` only after `ast_grep_search` is complete, cataloged, documented, and parity-tested.
- [ ] `ast_grep_replace` must default to dry-run.
- [ ] `ast_grep_replace` must require explicit apply mode for mutation.
- [ ] `ast_grep_replace` must go through edit permission checks.
- [ ] `ast_grep_replace` must enforce workspace path safety.
- [ ] `ast_grep_replace` must write diff/artifact evidence before apply.
- [ ] `ast_grep_replace` must avoid overlapping replacement hazards.
- [ ] `ast_grep_replace` must be tested for dry-run, apply, no-match, parse error, traversal rejection, permission denial, and artifact output.
- [ ] If any replace safety gate cannot be implemented cleanly, do not ship mutation. Document `ast_grep_replace` as deferred, keep roadmap unchecked for replace, and still complete required `ast_grep_search`.

### 4.8 Phase 8: Doctor, Support Export, TUI, And Docs Consumers

- [ ] Doctor JSON consumes the agent catalog for resolved profile/category/hidden metadata.
- [ ] Doctor JSON consumes the native tool catalog for tool ids, permission kinds, schema status, actor availability, and docs status.
- [ ] Doctor reports model-visible session tool readiness.
- [ ] Doctor reports `background_cancel` readiness.
- [ ] Doctor reports primitive `team_list` readiness and active team count if inspectable.
- [ ] Doctor reports AST-grep readiness without requiring network calls.
- [ ] Support export includes agent catalog summary and native tool catalog summary.
- [ ] Support export includes session tool availability and redaction status.
- [ ] TUI command/help/keybinding/slash metadata is centralized or adapted to consume one metadata table rather than maintaining conflicting strings.
- [ ] TUI remains compose-first and transcript-first.
- [ ] TUI status/help surfaces do not become runtime authority.
- [ ] README, `docs/config.md`, `docs/architecture.md`, `docs/testing.md`, and roadmap references are updated to exactly match shipped behavior.
- [ ] Add a concise native tool catalog doc in `docs/`.
- [ ] Add or update an agent/subagent guide in `docs/`.
- [ ] Add or update a sessions/replay guide in `docs/`.
- [ ] Add or update a permissions guide if tool-permission mapping changes or becomes newly documented.
...
- [ ] Agent catalog tests pass.
- [ ] Tool catalog parity tests pass.
- [ ] Session tool tests pass.
- [ ] Background cancellation tests pass.
- [ ] Team list tests pass.
- [ ] AST-grep search tests pass.
- [ ] AST-grep replace tests pass only if replace is implemented.
- [ ] Doctor text and JSON evidence are captured.
- [ ] Docs drift tests pass.
- [ ] Redaction tests pass for session/support output touched by this slice.
- [ ] TUI tests pass if command/help/keybinding/status surfaces change.
- [ ] Lane artifact paths are recorded for final report.

### 7.3 Documentation Deliverables

- [ ] Update `docs/roadmap-v1.md` checkboxes only when behavior is implemented and verified.
- [ ] Update `docs/architecture.md` for new catalog/tool/session contracts and any event/schema changes.
- [ ] Update `docs/config.md` for permission/tool/profile docs affected by this slice.
- [ ] Update `docs/testing.md` for new required targeted tests or lane evidence.
- [ ] Add `docs/native-tool-catalog.md` or equivalent concise V1 native tool catalog.
- [ ] Add or update `docs/sessions-and-replay.md` or equivalent model-visible session tool guide.
- [ ] Add or update `docs/agents-and-subagents.md` or equivalent agent catalog guide.
- [ ] Add or update permission docs if tool-permission mapping changes or becomes newly documented.
- [ ] README mentions only shipped behavior and does not claim full Team Mode or AST replace if deferred.
...
- [ ] Do not count doctor as provider execution proof.
- [ ] Do not count CLI session commands as model-visible session tools.
- [ ] Do not shell out from native tools to `harness sessions`.
- [ ] Do not count a docs table as a catalog seam.
- [ ] Do not let TUI command lists become the source of truth.
- [ ] Do not count `background_output(cancel=true)` alone as the dedicated `background_cancel` tool.
- [ ] Do not implement `team_list` by expanding Team Mode.
- [ ] Do not implement AST-grep replace before search, catalog, parity, permission, path-safety, and dry-run gates.
- [ ] Do not ship AST-grep mutation without explicit apply mode and edit permission.
- [ ] Do not add broad compatibility aliases that bypass canonical tool ids or permission names.
- [ ] Do not weaken tests, delete failing tests, narrow assertions, or mark roadmap boxes complete without evidence.
- [ ] Do not publish release claims for speed, provider breadth, parity, Team Mode, or AST replace unless current evidence proves them.
...
- [ ] Completed and remaining PRD checkboxes.
- [ ] Exact files touched.
- [ ] Exact tests and commands run.
- [ ] Artifact/evidence paths.
- [ ] Current failures with output summaries.
- [ ] Deferred conditional items, especially `ast_grep_replace`.
- [ ] Whether `team_list` stayed within primitive projection-reader scope.
- [ ] Open design decisions and why they block.
- [ ] Next smallest action for a fresh agent.

Do not write a final success report until all required gates pass.

## 13. Final Evidence Report Template

Use this template at closeout.

### Summary

- Slice: V1 Agent Catalog + Workspace Intelligence Control Plane
- Result: PASS / BLOCKED
- Commit range: `<range or uncommitted>`
- Evidence root: `<path>`
- Conditional `ast_grep_replace`: IMPLEMENTED / DEFERRED
- `team_list` scope: PRIMITIVE PROJECTION-READER CONFIRMED / BLOCKED

### Required Commands

| Command | Evidence path | Status |
|---|---|---|
| `cargo fmt --all -- --check` | `<path>` | PASS/FAIL |
| `cargo check --workspace` | `<path>` | PASS/FAIL |
| `cargo clippy --workspace --all-targets --all-features -- -D warnings` | `<path>` | PASS/FAIL |
| `cargo test --workspace --all-features` | `<path>` | PASS/FAIL |
| `cargo run -p harness -- --config configs/harness.example.jsonc config validate` | `<path>` | PASS/FAIL |
| `cargo run -p harness -- --config configs/harness.example.jsonc doctor --json` | `<path>` | PASS/FAIL |
| `cargo test -p harness-tools --test native_tool_parity_matrix_test` | `<path>` | PASS/FAIL |
| `cargo test -p harness --test replay_sessions_cli_test` | `<path>` | PASS/FAIL |
| `cargo test -p harness --test config_docs_reference_test` | `<path>` | PASS/FAIL |
| `cargo test -p harness --test event_docs_reference_test` | `<path>` | PASS/FAIL |
| `cargo test -p harness-tui` | `<path>` | PASS/FAIL |
| `scripts/test-lanes.sh fast` | `<path>` | PASS/FAIL |
| `scripts/test-lanes.sh quality-gates` | `<path>` | PASS/FAIL |

### Feature Evidence

| Feature | Tests/evidence | Status |
|---|---|---|
| Agent catalog seam | `<path>` | PASS/FAIL |
| Native tool catalog seam | `<path>` | PASS/FAIL |
| `session_list` | `<path>` | PASS/FAIL |
| `session_read` | `<path>` | PASS/FAIL |
| `session_search` | `<path>` | PASS/FAIL |
| `session_info` | `<path>` | PASS/FAIL |
| `background_cancel` | `<path>` | PASS/FAIL |
| `team_list` primitive | `<path>` | PASS/FAIL |
| `ast_grep_search` | `<path>` | PASS/FAIL |
| `ast_grep_replace` conditional | `<path or deferred rationale>` | PASS/DEFERRED/FAIL |
| Doctor/support metadata | `<path>` | PASS/FAIL |
| TUI metadata alignment | `<path or not touched>` | PASS/NOT TOUCHED/FAIL |
| Docs drift | `<path>` | PASS/FAIL |

### Deferred Or Env-Gated Items

- `ast_grep_replace`: `<implemented or deferred with safety reason>`
- Live provider lanes: `<used or not used for claims>`
- Native visual lanes: `<used or not used for claims>`
- Full Team Mode: out of scope

### Release Claim Boundary

State exactly what can be claimed:
...
- [ ] Every required checkbox in Sections 4 through 8 is complete.
- [ ] `team_list` exists as a narrow primitive projection-reader or the slice is marked blocked.
- [ ] `ast_grep_search` exists with strict schema, path safety, caps, artifacts, docs, and tests.
- [ ] `ast_grep_replace` is either implemented with all safety gates and evidence or explicitly deferred as conditional/stretch.
- [ ] Model-visible session tools exist and are replay-safe, redacted, capped, and tested.
- [ ] `background_cancel` exists and reuses coordinator-owned cancellation.
- [ ] Agent catalog metadata is reused by doctor/task/support/TUI consumers where touched.
- [ ] Native tool catalog metadata is parity-tested and documented.
- [ ] Docs and roadmap reflect exactly what shipped.
- [ ] Required commands pass or failures are documented as blockers.
- [ ] Final evidence report cites artifact paths, not only prose.
- [ ] No out-of-scope orchestration, plugin, browser/media, skill-MCP, or Team Mode work was smuggled into the slice.
