//! Jujutsu (`jj`) availability, workspace detection, and VCS workflow operations.
//!
//! Reports whether the `jj` binary is present on PATH and whether a workspace
//! looks like a jujutsu repo (`.jj` marker). Drives real `jj` commands for
//! status, log, new, describe, and bookmark workflows when the CLI is available.
...
/// Structured result of a jujutsu VCS workflow operation.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "status", rename_all = "snake_case")]
pub enum JujutsuWorkflowResult {
    Ok { operation: String, stdout: String },
    Unavailable { operation: String, reason: String },
}

impl JujutsuWorkflowResult {
    pub const fn is_ok(&self) -> bool {
        matches!(self, Self::Ok { .. })
    }
    ...
    pub fn one_line(&self) -> String {
        match self {
            Self::Ok { operation, stdout } => {
                let preview = stdout.trim();
                if preview.is_empty() {
                    format!("jujutsu {operation}: ok")
                } else {
                    format!("jujutsu {operation}: ok → {preview}")
                }
            }
            Self::Unavailable { operation, reason } => {
                format!("jujutsu {operation}: unavailable ({reason})")
            }
        }
    }
}
...
fn run_jj_workflow(
    ...
    operation: &str,
    args: &[&str],
) -> JujutsuWorkflowResult {
    let binary = match &probe.cli {
        JujutsuAvailability::Available { binary_path, .. } => binary_path.clone(),
        JujutsuAvailability::Unavailable { reason } => {
            return JujutsuWorkflowResult::Unavailable {
        ...
        .output()
    {
        ...
        Err(err) => {
            return JujutsuWorkflowResult::Unavailable {
                operation: operation.to_string(),
                reason: format!("failed to spawn jj: {err}"),
            };
        }
    };
    if !output.status.success() {
        ...
        let reason = if stderr.trim().is_empty() {
            ...
            format!("jj exited with status {}: {}", output.status, stderr.trim())
        };
        return JujutsuWorkflowResult::Unavailable {
            operation: operation.to_string(),
            reason,
...
pub fn jj_status(workspace_root: &Path) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_status_with_probe(&probe, workspace_root)
}

/// Injectable `jj status` for tests.
pub fn jj_status_with_probe(probe: &JujutsuProbe, workspace_root: &Path) -> JujutsuWorkflowResult {
    run_jj_workflow(probe, workspace_root, "status", &["status"])
}

/// `jj log` — show change log with an optional limit.
pub fn jj_log(workspace_root: &Path, limit: Option<u32>) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_log_with_probe(&probe, workspace_root, limit)
}

/// Injectable `jj log` for tests.
pub fn jj_log_with_probe(
    probe: &JujutsuProbe,
    workspace_root: &Path,
    limit: Option<u32>,
) -> JujutsuWorkflowResult {
    ...
    run_jj_workflow(probe, workspace_root, "log", &args)
}

/// `jj new` — create a new change with an optional message.
pub fn jj_new(workspace_root: &Path, message: Option<&str>) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_new_with_probe(&probe, workspace_root, message)
}

/// Injectable `jj new` for tests.
pub fn jj_new_with_probe(
    probe: &JujutsuProbe,
    workspace_root: &Path,
    message: Option<&str>,
) -> JujutsuWorkflowResult {
    ...
    run_jj_workflow(probe, workspace_root, "new", &args)
}

/// `jj describe` — describe the current change with a message.
pub fn jj_describe(workspace_root: &Path, message: &str) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_describe_with_probe(&probe, workspace_root, message)
}

/// Injectable `jj describe` for tests.
pub fn jj_describe_with_probe(
    probe: &JujutsuProbe,
    workspace_root: &Path,
    message: &str,
) -> JujutsuWorkflowResult {
...
/// `jj bookmark create` — create a bookmark (jj's branch equivalent).
pub fn jj_bookmark_create(workspace_root: &Path, name: &str) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_bookmark_create_with_probe(&probe, workspace_root, name)
}

/// Injectable `jj bookmark create` for tests.
pub fn jj_bookmark_create_with_probe(
    probe: &JujutsuProbe,
    workspace_root: &Path,
    name: &str,
) -> JujutsuWorkflowResult {
...
pub fn jj_bookmark_list(workspace_root: &Path) -> JujutsuWorkflowResult {
    let probe = probe_jujutsu(workspace_root);
    jj_bookmark_list_with_probe(&probe, workspace_root)
}

/// Injectable `jj bookmark list` for tests.
pub fn jj_bookmark_list_with_probe(
    probe: &JujutsuProbe,
    workspace_root: &Path,
) -> JujutsuWorkflowResult {
    run_jj_workflow(
...
mod tests {
    ...
    fn make_fake_jj(dir: &Path) -> PathBuf {
        let fake = dir.join("jj");
        std::fs::write(
            &fake,
            "#!/bin/sh\ncase \"$1\" in\n  status) echo 'Working copy changes:';;\n  log) echo '@  test-change';;\n  new) echo 'Created new change';;\n  describe) echo 'Described change';;\n  bookmark) case \"$2\" in\n    create) echo \"Created bookmark $3\";;\n    list) echo 'main@';;\n    *) exit 1;;\n  esac;;\n  *) exit 1;;\nesac\n",
        )
        .unwrap();
        #[cfg(unix)]
        {
    ...
    fn fake_probe(dir: &Path) -> (JujutsuProbe, PathBuf) {
        let fake = make_fake_jj(dir);
    ...
    #[test]
    fn jj_status_workflow_returns_ok_with_fake_binary() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let (probe, workspace) = fake_probe(dir.path());

        // act
        let result = jj_status_with_probe(&probe, &workspace);

        // assert
        assert!(result.is_ok(), "{}", result.one_line());
        assert!(result.one_line().contains("status"));
    }

    #[test]
    fn jj_log_workflow_returns_ok_with_limit() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let (probe, workspace) = fake_probe(dir.path());

        // act
        let result = jj_log_with_probe(&probe, &workspace, Some(5));

        // assert
        assert!(result.is_ok(), "{}", result.one_line());
        assert!(result.one_line().contains("log"));
    }

    #[test]
    fn jj_new_workflow_with_message_returns_ok() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let (probe, workspace) = fake_probe(dir.path());

        // act
        let result = jj_new_with_probe(&probe, &workspace, Some("test message"));

        // assert
        assert!(result.is_ok(), "{}", result.one_line());
        assert!(result.one_line().contains("new"));
    }

    #[test]
    fn jj_describe_workflow_returns_ok() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let (probe, workspace) = fake_probe(dir.path());

        // act
        let result = jj_describe_with_probe(&probe, &workspace, "described");

        // assert
        assert!(result.is_ok(), "{}", result.one_line());
        assert!(result.one_line().contains("describe"));
    }

    #[test]
    fn jj_bookmark_create_and_list_return_ok() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let (probe, workspace) = fake_probe(dir.path());

        // act
        let create = jj_bookmark_create_with_probe(&probe, &workspace, "feature");
        let list = jj_bookmark_list_with_probe(&probe, &workspace);

        // assert
        assert!(create.is_ok(), "{}", create.one_line());
        assert!(list.is_ok(), "{}", list.one_line());
        assert!(create.one_line().contains("bookmark_create"));
        assert!(list.one_line().contains("bookmark_list"));
    }

    #[test]
    fn jj_workflow_fails_closed_when_cli_unavailable() {
        // arrange
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path().join("ws");
        std::fs::create_dir_all(workspace.join(".jj")).unwrap();
