<div align="center">
  ...
  <p>
    ...
    <a href="docs/config.md">Reference</a>
  </p>
</div>
...
Use one live `run` or interactive turn to prove provider authentication and transport. If that fails, start with [`doctor`](docs/troubleshooting.md) and the [provider support guide](docs/provider-support.md).
...
The [full config reference](docs/config.md) documents every public key and its validation behavior. For the exact permission vocabulary and ruleset semantics, read the [permissions guide](docs/permissions.md).
...
See the [TUI configuration reference](docs/config.md#tui-top-level-keys) for all action ids and default bindings.
...
The [agents and subagents guide](docs/agents-and-subagents.md) explains the shipped profiles, category fallback, delegation-body shape, and the runtime boundaries that prevent worker redelegation bypasses.
...
Use a support export rather than sharing raw events. It contains replay-derived metadata, a redaction manifest, non-secret configuration summaries, and a secret-scan result. Learn more in [sessions and replay](docs/sessions-and-replay.md) and [privacy and local data](docs/privacy-and-local-data.md).
...
For capabilities and replay behavior tool by tool, see the [native tool catalog](docs/native-tool-catalog.md). For important limits—such as why replay never executes a tool—see the [architecture](docs/architecture.md).
...
The [testing and signoff map](docs/testing.md) explains exactly what each lane proves—and, just as importantly, what it does not prove.
...
The [first-run troubleshooting guide](docs/troubleshooting.md) contains the longer diagnosis paths.

## Explore the project

| Area | Start here |
| --- | --- |
| Configuration and providers | [Config reference](docs/config.md) · [Provider support](docs/provider-support.md) |
| Tools and permissions | [Native tool catalog](docs/native-tool-catalog.md) · [Permissions](docs/permissions.md) |
| Sessions and recovery | [Sessions and replay](docs/sessions-and-replay.md) · [Privacy](docs/privacy-and-local-data.md) |
| Architecture | [Crate boundaries and invariants](docs/architecture.md) |
| Testing | [Testing and signoff](docs/testing.md) |
