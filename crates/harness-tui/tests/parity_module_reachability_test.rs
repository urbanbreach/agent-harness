...
fn checked_in_inventory_proves_every_parity_module_reaches_product() {
    ...
    let inventory: Inventory = serde_json::from_str(INVENTORY).expect("inventory must be valid JSON");
    ...
    for module in inventory.modules {
        assert!(
            matches!(module.classification.as_str(), "live_reachable" | "internal_to_live_owner"),
