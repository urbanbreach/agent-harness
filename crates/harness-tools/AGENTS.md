...
| Task | Location | Notes |
|------|----------|-------|
| Registry/catalog | `src/lib.rs`, `src/tool_catalog.rs`, `src/tool_catalog/baseline_parity.rs` | `coordinator_registry*`/`worker_registry*` builders, executor wiring, `NativeToolCatalogEntry` metadata, baseline parity status. |
