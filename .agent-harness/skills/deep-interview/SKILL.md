...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what Harness may decide without confirmation.
</Purpose>
...
- Use these transcript/spec labels only; never use them as `native question tool` `source` values, and never replace the runtime `source: "deep-interview"` contract for user-facing deep-interview questions:
...
- Treat `[from-code][auto-confirmed]` and other non-user fact discoveries as context/transcript updates, not interview rounds: do not call `native question tool`, do not create a pending deep-interview question obligation, and do not increment the user-facing round number for facts the agent can safely establish.
...
- In attached-native terminal UI Codex CLI, deep-interview uses `native question tool` as the required Harness-owned structured questioning path for every interview round
- When invoking `native question tool` through attached-native terminal UI Bash/tool paths, preserve the leader-pane return target by prefixing the command with `HARNESS_QUESTION_SETTING_RETURN_PANE=$HARNESS_TERMINAL_PANE` (or a concrete `%pane` value)
- If you launch `native question tool` in a background terminal, immediately wait for that background terminal to finish and read its JSON answer before scoring ambiguity, asking another round, or handing off
- Treat `answers[]` as the primary `native question tool` success contract. For a single interview round, read `answers[0].answer`; use legacy top-level `answer` only as a compatibility fallback when needed.
- If the current runtime is outside native terminal UI and cannot render `native question tool`, use the native structured question tool when available; otherwise ask exactly one concise plain-text question and wait for the answer
...
- Persist mode state for resume safety with CLI-first state commands (`harness workflow evidence write/read --input '<json>' --json`); use `state_write` / `state_read` only when explicit MCP compatibility is enabled
</Execution_Policy>
...
3. Initialize state via `harness workflow evidence write --input '{"mode":"deep-interview","active":true}' --json`:
...
### 2b) Ask the question
Use the surface-appropriate structured questioning path for every interview round. In attached-native terminal UI sessions, use Harness-owned structured questioning via `native question tool` (this is the required structured-question equivalent and required `AskUserQuestion` equivalent for deep-interview). Outside native terminal UI, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer. Present:
...
`native question tool` payload guidance for interview rounds:
- Deep-interview is Socratic: ask one focused round at a time. Do not use batch `questions[]` to combine multiple interview rounds, even though `native question tool` supports batch forms for other workflows.
...
### 2e) Persist state
Append round result and updated scores via `harness workflow evidence write --input '<json>' --json`; use `state_write` only when explicit MCP compatibility is enabled.
...
- Decision Boundaries (what Harness may decide without confirmation)
- Constraints
...
- **Confirmation bridge:** after artifact generation, offer at least `refine further` and `launch`; do not run direct CLI launch or detached/split native terminal UI launch, and only hand off to `$autoresearch` after explicit confirmation
...
- **`$ultragoal`** — default goal-mode follow-up for implementation or general goal-oriented follow-up specs that should be converted into durable Codex/Harness goals with sequential completion tracking.
...
- Use `native question tool` as the Harness-native structured user-input tool for each interview round when an attached native terminal UI renderer is available
- From attached-native terminal UI Bash/tool paths, call it as `HARNESS_QUESTION_SETTING_RETURN_PANE=$HARNESS_TERMINAL_PANE native question tool ...` unless an explicit `%pane` return target is already known
- If the current runtime is outside native terminal UI and cannot render `native question tool`, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer
...
If interrupted, rerun `$deep-interview`. Resume from persisted mode state via `harness workflow evidence read --input '{"mode":"deep-interview"}' --json`.
