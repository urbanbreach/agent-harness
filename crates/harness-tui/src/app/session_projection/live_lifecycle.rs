use super::*;
use harness_core::event::ProviderRequestStartedEvent;
...
impl SessionProjection {
    pub(super) fn update_live_lifecycle_event(
        ...
        match &event.payload {
            ...
            EventV1::ProviderRequestStarted(data) => {
                self.update_live_provider_started(event, data);
            }
            EventV1::ProviderRequestFinished(data) => {
                self.update_live_provider_finished(event, data);
            }
            _ => return false,
        }
        true
    ...
    fn update_live_provider_started(
        &mut self,
        event: &EventEnvelopeV1,
        data: &ProviderRequestStartedEvent,
    ) {
        let is_latest = self
    ...
    fn update_live_provider_finished(
        &mut self,
        event: &EventEnvelopeV1,
        ...
        if let Some(index) = self.activity_index_for_provider_event(event, data.request_id.as_str())
        {
            ...
            if let Some(entry) = self.activities.get_mut(index) {
                ...
                if let Some(usage) = data.usage.as_ref() {
                    ...
                    let context_tokens = match usage.prompt_tokens {
                        0 => usage.total_tokens,
                        tokens => tokens,
                    };
                    self.active_context_usage = Some(ActiveContextUsage::estimate(context_tokens));
                }
