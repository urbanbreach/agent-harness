...
fn safe_event_summary(event: &EventEnvelopeV1) -> Value {
    match &event.payload {
...
fn safe_message_summaries(entry: &SessionEntry) -> Vec<Value> {
    entry
        .events
        .iter()
        .filter(|event| {
...
fn safe_search_documents(entry: &SessionEntry) -> Vec<SearchDocument> {
...
fn event_counts_by_type(events: &[EventEnvelopeV1]) -> BTreeMap<String, u64> {
