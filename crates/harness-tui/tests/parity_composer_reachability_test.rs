...
fn production_composer_reaches_keyboard_paste_completion_queue_attachment_and_submit() {
    ...
    assert_eq!(app.composer_view_model(harness_tui::design_contract::ViewportId::Standard100x30).text, "hello");
    ...
    app.composer_apply_completion_results(
        &request,
        vec![CompletionItem::new(1, "hello", "hi")],
    )
...
fn replay_and_provider_transport_preserve_attachment_contract() {
    ...
    assert_eq!(serialized[0].metadata().content_ref, "attachment:v1:content-redacted");
}
