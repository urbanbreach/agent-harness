...
| `harness_tui_p1_03_pty_recorded` | serialized P1-03 native PTY owner for the staged startup reveal: first-seen identity/affordance/changelog ordering, reduced-motion freeze on the complete frame, input dismissal during the reveal, and Unicode + Basic/Ascii artifacts at 80x24/120x40/160x50 |
...
| `p1_03_xterm_80x24`, `p1_03_xterm_120x40`, `p1_03_xterm_160x50` | the exact compiled P1-03 integration-test owner executed in Chromium+xterm.js with the `p1-03-startup-reveal` contract (welcome-complete and after-input captures) at each canonical size |
