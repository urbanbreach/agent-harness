...
- If persisted scope is `project`, `harness` launch automatically uses `HARNESS_HOME=./.codex` unless user explicitly overrides `HARNESS_HOME`.
...
## If `$setup` is missing or stale

The source repo ships `skills/setup/SKILL.md` and the catalog marks it active. If Codex does not show `$setup`, treat it as an installation/discovery issue rather than a missing source skill:
...
3. If using project scope, confirm `./.codex/skills/setup/SKILL.md` exists.
4. If using user scope, confirm `configured Harness home/skills/setup/SKILL.md` exists in legacy mode, or that the Agent Harness plugin is installed/discovered in plugin mode.
...
- If your global `harness` points to another install, run local entrypoint:
