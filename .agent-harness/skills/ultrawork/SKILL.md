...
Harness records Ultrawork lifecycle, phase, continuation, and evidence through coordinator-owned events and workflow projections. Do **not** run `harness workflow evidence` or write per-mode `Harness workflow projection/*.json` files inside the harness.

- **On start**: `$ultrawork` dispatch records a workflow start event plus initial workflow evidence.
- **On each reinforcement/loop step**: the active workflow context overlay re-injects mode/phase/continuation guidance before provider submission.
- **On progress**: record material outcomes as workflow evidence or task/team evidence artifacts.
- **On completion/cancellation**: use harness workflow signoff/cancel surfaces; completion remains bounded to direct Ultrawork's lightweight verification contract.
