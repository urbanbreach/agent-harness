...
fn redaction_marker_for_sensitive_key(key: &str) -> Option<&'static str> {
    ...
    for character in key.chars().filter(|c| c.is_ascii_alphanumeric()) {
...
fn adjacent_segments<'a>(segments: impl Iterator<Item = &'a str>, left: &str, right: &str) -> bool {
    let mut prev_was_left = false;
    for segment in segments {
        if prev_was_left && segment.eq_ignore_ascii_case(right) {
            return true;
        }
        prev_was_left = segment.eq_ignore_ascii_case(left);
    }
    false
}
