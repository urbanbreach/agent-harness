...
fn all_profile_deduplicates_six_obligations_to_three_verification_keys() {
    // arrange: six typed obligations sharing two non-runtime keys and one capture key.
    ...
    // act: obligations are converted into executable verification keys.
    let plan = deduplicate_obligations(&inventory, &manifest, &selected)
        .expect("synthetic obligations must deduplicate");

    // assert: each execution-effective key appears exactly once.
...
fn obligation_type_is_exhaustively_deserialized_from_inventory() {
    // arrange: the synthetic inventory contains every non-reviewer obligation type.
    let (inventory, _) = synthetic_fixture();

    // act: the typed records are inspected.
    ...
    // assert: no record fell through an untyped compatibility default.
    assert_eq!(
        observed,
        BTreeSet::from([
...
fn dependency_cone_selects_scenario_obligations_for_known_path() {
    // arrange: a path mapped transitively to one scenario family.
    let (inventory, manifest) = synthetic_fixture();
    ...
    // act: a source path in that cone changes.
    let selection = cones
        .select(
    ...
    // assert: all six requirements sharing the affected scenario are selected.
...
fn dependency_cone_fails_closed_for_unknown_path() {
    // arrange: a dependency map with no entry for a new source path.
    let (inventory, manifest) = synthetic_fixture();
    let cones = DependencyCones::from_json(
    ...
    // act: selection sees an unknown path.
    let selection = cones
        .select(&["new/unknown.rs".into()], &inventory, &manifest)
        .expect("fail-closed selection");

    // assert: the full inventory is selected and the path is recorded.
    assert!(selection.fell_back_to_all);
    assert_eq!(selection.requirement_ids.len(), 6);
...
fn synthetic_fixture_runs_through_all_changed_and_motion_profiles() {
    // arrange: six obligations collapsing to two non-runtime keys and one motion key.
    let (inventory, manifest) = synthetic_fixture();
    ...
    // act: every verification profile builds its execution plan.
    let plans = [
        build_plan(
            PlanSelection {
    ...
    // assert: all/changed retain static work while motion is dynamic-only.
